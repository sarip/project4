            ...
            <div class="col-lg-6" style="border-left: 1px solid grey;">
              ...
              <?php foreach ($dataJurusan as $jurusan): ?>
                <?php foreach ($dataKelas as $kelas): ?>
                  <big class="text-center"><?= '<b>'.$kelas->nama_kelas .'</b> <big>'.$jurusan->nama_jurusan.'</big>'?><br></big>
                    <?php foreach ($dataPelajaran as $pelajaran): ?>
                      <?php if ($this->db->get_where('mengajar', ['id_guru' => $guru->id_guru, 'id_kelas' => $kelas->id_kelas, 'id_jurusan' => $jurusan->id_jurusan, 'id_pelajaran' => $pelajaran->id_pelajaran])->num_rows() > 0): ?>
                        <input type="checkbox" 
                      name="id_jurusan_<?= $jurusan->id_jurusan ?>_id_kelas_<?= $kelas->id_kelas ?>_id_pelajaran_<?= $pelajaran->id_pelajaran  ?>" id="id_jurusan_<?= $jurusan->id_jurusan ?>_id_kelas_<?= $kelas->id_kelas ?>_id_pelajaran_<?= $pelajaran->id_pelajaran  ?>" id="<?= $pelajaran->id_pelajaran ?>"
                      value="<?= $kelas->id_kelas.','.$jurusan->id_jurusan.','.$pelajaran->id_pelajaran  ?>" checked> 
                        <label for="id_jurusan_<?= $jurusan->id_jurusan ?>_id_kelas_<?= $kelas->id_kelas ?>_id_pelajaran_<?= $pelajaran->id_pelajaran  ?>" id="<?= $pelajaran->id_pelajaran ?>"><?= $pelajaran->nama_pelajaran ?></label><br>
                      <?php else: ?>
                      <input type="checkbox" 
                      name="id_jurusan_<?= $jurusan->id_jurusan ?>_id_kelas_<?= $kelas->id_kelas ?>_id_pelajaran_<?= $pelajaran->id_pelajaran  ?>" id="id_jurusan_<?= $jurusan->id_jurusan ?>_id_kelas_<?= $kelas->id_kelas ?>_id_pelajaran_<?= $pelajaran->id_pelajaran  ?>" id="<?= $pelajaran->id_pelajaran ?>"
                      value="<?= $kelas->id_kelas.','.$jurusan->id_jurusan.','.$pelajaran->id_pelajaran  ?>"> 
                        <label for="id_jurusan_<?= $jurusan->id_jurusan ?>_id_kelas_<?= $kelas->id_kelas ?>_id_pelajaran_<?= $pelajaran->id_pelajaran  ?>" id="<?= $pelajaran->id_pelajaran ?>"><?= $pelajaran->nama_pelajaran ?></label><br>
                      <?php endif ?>
                    <?php endforeach ?>
                    <br>
